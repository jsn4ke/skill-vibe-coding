...
type TargetResolver func() []uint64

func (m *Manager) TickPeriodicArea(elapsed time.Duration, casterSpellPower float64, resolveTargets TargetResolver, onTick func(*Aura, *AuraEffect, float64, uint64)) {
	var expired []*Aura
	for _, list := range m.auras {
		for _, a := range list {
			if !a.IsAreaAura {
...
func (m *Manager) TickPeriodic(targetID uint64, elapsed time.Duration, casterSpellPower float64, onTick func(*Aura, *AuraEffect, float64)) {
	var expired []*Aura
	for _, a := range m.auras[targetID] {
		a.Elapsed += elapsed
		for i := range a.Effects {
